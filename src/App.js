...
export default function App() {
  ...
  const initialMintHelper = async () => {
   try {
       ...
       await creatorToken.mintTo(fromTokenAccount.address, mintingFromWallet.publicKey, [], 100000000);
...
const mintAgainHelper=async () => {
   try {
       ...
       const mintRequester = "7V8ztSoXVrVmZnU1r4jJdbqtDSSnGmGgdJWsCvJ1qasg";
    ...
    <div>
      ...
      {walletConnected ? (
        <div>
          ...
          <p>Mint More 100 tokens: <button disabled={loading || supplyCapped} onClick={mintAgainHelper}>Mint Again</button></p>
          <p>Transfer tokens to your friends: <button disabled={loading} onClick={transferTokenHelper}>Transfer 10 tokens</button></p>
          <p>Cap token supply: <button disabled={loading} onClick={capSupplyHelper}>Cap token supply</button></p>
        </div>
      ) : (
        <p></p>
      )}
